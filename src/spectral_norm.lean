...
lemma max_repeat {α : Type*} {n : ℕ} (a : α) [linear_order α] :
  foldr max a (repeat a n) = a :=
begin
...
lemma le_max_of_exists_le {α : Type*} [linear_order α] {l : list α} {a x : α} (b : α)
...
lemma C_finset_add {α : Type*} (s : finset α) (b : α → K) :
...
lemma C_finset_prod {α : Type*} (s : finset α) (b : α → K) :
...
begin
  rw [multiset.esymm, powerset_len_map, ← finset.map_val_val_powerset_len],
  simpa only [map_map],
end
 -/
-- (map (λ (a : L), X - ⇑C a) s).prod
...
begin
  ...
  { apply csupr_le,
    rintros x,
    by_cases hx : x ∈ s,
    { have hx0 : aeval x p = 0 := aeval_root s hx hp,
      rw if_pos hx,
...
lemma spectral_norm_ne_zero_of_map_ne_zero (h_alg : algebra.is_algebraic K L) {x : L}
...
begin
  ...
  { set p := minpoly K x with hp_def,
    ...
    { have hy : ∃ σ : L ≃ₐ[K] L, σ x = y,
      { exact minpoly.conj_of_root' h_alg hn (aeval_root s h hs), },
      obtain ⟨σ, hσ⟩ := hy,
      rw ← hσ,
...
lemma spectral_norm_smul (h_alg : algebra.is_algebraic K L) (hna : is_nonarchimedean (norm : K → ℝ)) (k : K) (y : L) :
  spectral_norm K L (k • y) = ‖ k ‖₊ * spectral_norm K L y :=
begin
  set E := K⟮y⟯ with hE,
